...
// TODO: add form feature and connect the city const to it 
...
try {
const result = await axios.get(`http://pro.openweathermap.org/data/2.5/weather?${city}&units=metric${API_Key}${language}`);
...
const iconResponse = await axios.get(`https://openweathermap.org/img/wn/${iconID}@2x.png`, {
    responseType : "arraybuffer"
});

const iconBase64 = btoa(
    new Uint8Array(iconResponse.data).reduce((data, byte) => data + String.fromCharCode(byte), '')
);
const iconDataUrl = `data:image/png;base64,${iconBase64}`;

const AQIIndex = await axios.get(`http://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}${API_Key}`);
...
const AQI_PM2 = bp_1_pm2 + ((bp_2_pm2 - bp_1_pm2) / (bp_2_pm2 - bp_1_pm2)) * (pm2 - bp_1_pm2);
const AQI_PM10 = bp_1_pm10 + ((bp_2_pm10 - bp_1_pm10) / (bp_2_pm10 - bp_1_pm10)) * (pm10 - bp_1_pm10);
const AQI_o3 = bp_1_o3 + ((bp_2_o3 - bp_1_o3) / (bp_2_o3 - bp_1_o3)) * (o3 - bp_1_o3);
